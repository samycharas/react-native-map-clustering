...
const ClusteredMapView = forwardRef(
  (
    ...
    const getAllChildren = (cluster) => {
      let markers = [];
      markers.value = 0;
      markers.count = 0;
      let clusters = [];
      res = superCluster.getChildren(cluster.properties.cluster_id);

      for (let index = 0; index < res.length; index++) {
        const element = res[index];
        if (element.properties.cluster == true)
        {
          clusters.push(res[index])
        }
        else {
          markers.value += parseFloat(element.properties.identifier);
          markers.count += 1;
        }
      }
      while(clusters.length > 0)
      {
          const smallerCluster = clusters.shift();
          res = superCluster.getChildren(smallerCluster.properties.cluster_id);
          for (let index = 0; index < res.length; index++) {
            const element = res[index];
            if (element.properties.cluster == true)
            {
              ...
              markers.push(element.properties.identifier)
            }
          }
      }
      pmvalue = markers.value/markers.count;
      console.log(pmvalue)

      if (pmvalue < 4) {
        var bgColor = Colorscale.green1;
      }
      else if (pmvalue < 7) {
        var bgColor = Colorscale.green2;
      }
      else if (pmvalue < 10) {
        var bgColor = Colorscale.green3;
      }
      else if (pmvalue < 15) {
        var bgColor = Colorscale.yellow1;
      }
      else if (pmvalue < 24) {
        var bgColor = Colorscale.yellow2;
      }
      else if (pmvalue < 35) {
        var bgColor = Colorscale.yellow3;
      }
      else if (pmvalue < 50) {
        var bgColor = Colorscale.orange1;
      }
      else if (pmvalue < 90) {
        var bgColor = Colorscale.orange2;
      }
      else if (pmvalue < 150) {
        var bgColor = Colorscale.red1;
      }
      else if (pmvalue < 180) {
        var bgColor = Colorscale.red2;
      }
      else if (pmvalue < 250) {
        var bgColor = Colorscale.purple1;
      }
      else if (pmvalue < 800) {
        var bgColor = Colorscale.purple2;
      }
      else {
        var bgColor = Colorscale.gray;
      }
      return bgColor;
    }
          ...
          marker.properties.point_count === 0 ? (
            ...
            renderCluster ? (
              ...
              <ClusterMarker
                ...
                {...marker}
                onPress={_onClusterPress(marker)}
                clusterColor = {getAllChildren(marker)}
/*                 clusterColor={
